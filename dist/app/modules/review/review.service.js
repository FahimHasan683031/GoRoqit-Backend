...
const getAllReviews = async (user, type, paginationOptions) => {
    ...
    const [result, total] = await Promise.all([
        review_model_1.Review.find({ [type]: user.authId }).populate('reviewer').populate('reviewee').skip(skip).limit(limit).sort({ [sortBy]: sortOrder }),
        review_model_1.Review.countDocuments({ [type]: user.authId })
    ]);
    //cache the result 
