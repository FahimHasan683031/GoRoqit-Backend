...
exports.CustomAuthServices = exports.createUser = void 0;
...
const createUser = async (payload) => {
    ...
    try {
        ...
        await emailHelper_1.emailHelper.sendEmail(createAccountEmail);
        // 4. Create User
        const user = await user_model_1.User.create([{ ...payload, password: payload.password, authentication }], { session });
        if (!user[0])
        ...
        if (payload.role === user_1.USER_ROLES.APPLICANT) {
            ...
            updatedUser = await user_model_1.User.findByIdAndUpdate(createdUser._id, { roleProfile: "ApplicantProfile", profile: profile[0]._id }, { new: true, session });
        }
        else if (payload.role === user_1.USER_ROLES.RECRUITER) {
            const profile = await recruiterProfile_model_1.RecruiterProfile.create([{ userId: createdUser._id, companyName: payload.companyName }], { session });
            if (!profile[0])
                throw new ApiError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "Failed to create recruiter profile.");
            updatedUser = await user_model_1.User.findByIdAndUpdate(createdUser._id, { roleProfile: "RecruiterProfile", profile: profile[0]._id }, { new: true, session });
        }
